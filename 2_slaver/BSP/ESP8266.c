...
{
  if(ESP8266_SoftReset(50))return 1;                         //Èí¼þ¸´Î»8266£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü¾Í·µ»Ø1
  else printf("Èí¸´Î»³É¹¦\r\n");
  
  if(ESP8266_SendCmd_OK("AT+CWMODE=1",50))return 2;          //ÉèÖÃ8266ÎªSTAÄ£Ê½£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø2
  else printf("ÉèÖÃSTAÄ£Ê½³É¹¦\r\n");
  
  printf("Á¬½ÓAPÖÐ... ...\r\n");
  if(ESP8266_ConnectionAP(AP_SSID,AP_PASS,200))return 3;     //ÉèÖÃ8266Á¬½ÓAP£¬³¬Ê±Ê±¼ä20s£¬Ê§°Ü·µ»Ø3
  else printf("Á¬½ÓAP³É¹¦\r\n");
  
  if(ESP8266_GetIP(IP_Buff,50))return 4;                     //²éÑ¯8266µÄIP£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø4
  else printf("²éÑ¯IP³É¹¦\r\n");
  
  if(ESP8266_SendCmd_OK("AT+CIPMODE=0",50))return 5;         //¹Ø±ÕÍ¸´«£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø5
  else printf("¹Ø±ÕÍ¸´«³É¹¦\r\n");
  
  if(ESP8266_SendCmd_OK("AT+CIPMUX=1",50))return 6;          //¿ªÆô¶àÂ·Á¬½Ó£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø6
  else printf("¿ªÆô¶àÂ·Á¬½Ó³É¹¦\r\n");
  
  if(ESP8266_SendCmd_OK("AT+CIPSERVER=1,5050",50))return 7;  //¿ªÆôTCP·þÎñÆ÷£¬¶Ë¿Ú5050£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø7	
...
u8 ESP8266_STA_TCPCLient(void)
{
  ...
  if(ESP8266_SendCmd_OK("AT+CIPMODE=1",50))return 5;         //¿ªÆôÍ¸´«£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø5
  else printf("¿ªÆôÍ¸´«³É¹¦\r\n");

  if(ESP8266_SendCmd_OK("AT+CIPMUX=0",50))return 6;          //¹Ø±Õ¶àÂ·Á¬½Ó£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø6
  else //printf("¹Ø±Õ¶àÂ·Á¬½Ó³É¹¦\r\n");
...
u8 ESP8266_GetIP(u8 *ip, u16 timeout)
{
  ...
  else
  {
    ...
    if( presult1 != NULL )
    {
      ...
      else return 3;  //Î´ÊÕµ½Ô¤ÆÚÊý¾Ý
    }
    else return 2;  //Î´ÊÕµ½Ô¤ÆÚÊý¾Ý
  }		
}
 /**
...
u8 ESP8266_SendData(u8 id, u8 *databuff, u16 data_len, u16 timeout)
{
  ...
  else
  {
    USART2_RX_STA=0;                        
    memset(USART2_RX_BUF,0,USART2_REC_LEN);                  //Çå³ý´®¿Ú2Êý¾Ý	
    for(i=0;i<data_len;i++)u2_printf("%c",databuff[i]);      //·¢ËÍÊý¾Ý	
    time=0;
    while(time<timeout)
    {
